...
static void inline CAN_IT_ENABLE( u32 DATA) { CAN1->INTENR |= DATA; }
static void inline CAN_CLEAR_TRANSMI_FLAG( u32 DATA) { CAN1->TSTATR = (uint32_t)(DATA & 0x000FFFFF); }
#endif
...
HAL_CAN_ERROR_t HAL_CANGetRXMessage( HAL_CAN_RX_FIFO_NUMBER_t fifo,  CAN_FRAME_TYPE * rx_message )
{
   ...
   if ( rx.remoteTxReq != CAN_RTXR_REMOTE )
   {
    ...
    {
           u32   StdId = (uint32_t)0x000007FF & (CAN1->sFIFOMailBox[FIFONumber].RXMIR >> 21);
...
{
	CAN.rxcallback( HAL_RX_FIFO0);
	CAN1->RFIFO0 = CAN_RFIFO0_FULL0;
}
void   CAN1_RX1_IRQHandler(void)
{
	CAN.rxcallback( HAL_RX_FIFO1);
	CAN1->RFIFO1 = CAN_RFIFO1_FULL1;
}
void   CAN1_SCE_IRQHandler(void)
{
	CAN1->STATR = CAN_STATR_ERRI;
    CAN.errorcallback();
}
