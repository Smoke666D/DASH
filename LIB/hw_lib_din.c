...
INIT_FUNC_LOC DIN_FUNCTION_ERROR_t eDinConfigWtihStruct(DIN_INPUT_NAME ucCh, DinConfig_t * config)
{
    ...
    xDinConfig[ucCh].ucTempValue = (xDinConfig[ucCh].eInputType == DIN_CONFIG_POSITIVE ) ? 0U : 0U;
    xDinConfig[ucCh].ulCounter = 0;
    eRes = DIN_CONFIG_OK;
    return ( eRes );
...
void RMPDataConvert(DIN_INPUT_NAME ucCh)
{
   // printf("data_get\r\n");
    ...
    for (u8 i=1;i< CC_BUFFER_SIZE;i++)
    {
       ...
       xDinConfig[ucCh].RPMDATA->BufferData =xDinConfig[ucCh].RPMDATA->BufferData+ u16temp;
    }

    xDinConfig[ucCh].RPMDATA->BufferData = (xDinConfig[ucCh].RPMDATA->BufferData/CC_BUFFER_SIZE);
...
uint16_t GetRPM( DIN_INPUT_NAME ucCh )
{
    ...
    if (( xDinConfig[ucCh].RPMDATA != 0) && (xDinConfig[ucCh].eInputType==RPM_CONFIG))
    {
    	...
    	{
    		usTemp =(uint16_t)(1.0/ (double)(xDinConfig[ucCh].RPMDATA->BufferData * 0.0000333 ))*60;
